...
def main():
  ...
  for seed in range(0, 16):
    print("  seed %d ..."%seed)
    for file in glob.glob('static/100k/umaps/%d/*.json'%seed):
      ...
      # write
      points_for_seed = project_to_cluster_on_umap(points, seed=42)
      if not os.path.exists('static/100k/five_clusters_in_umap/%d'%seed):
        os.mkdir('static/100k/five_clusters_in_umap/%d'%seed)
      with open('static/100k/five_clusters_in_umap/%d/%s.json'%(seed,word), 'w') as outfile:
